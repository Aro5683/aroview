...
<script>
  // =========================================================
  // 1. STORAGE QUOTA MANAGER (Display Space Bar)
  // =========================================================
  async function updateStorageQuota() {
    if (navigator.storage && navigator.storage.estimate) {
      ...
      // Display format: MB used / GB total quota
      document.getElementById('storage-text').innerText = `${usageMB} MB / ${quotaGB} GB`;
      document.getElementById('storage-bar').style.width = `${percent}%`;
    }
  }

  // =========================================================
  // 2. OPFS DOWNLOAD ENGINE & STREAMING
  // =========================================================
  function getOpfsFileName(url) {
    let hash = 0;
  ...
  async function fetchAndStoreInOPFS(pdfUrl) {
    const fileName = getOpfsFileName(pdfUrl);
    const root = await navigator.storage.getDirectory();

    // A. Check if PDF is already stored in OPFS
    try {
      const existingFileHandle = await root.getFileHandle(fileName);
      const existingFile = await existingFileHandle.getFile();
      if (existingFile.size > 0) {
        console.log("Loading PDF directly from OPFS cache...");
    ...
    // B. Show Download Progress UI
    const modal = document.getElementById('opfs-progress-modal');
    const progressBar = document.getElementById('opfs-download-bar');
    const percentText = document.getElementById('opfs-download-percent');
    const statusText = document.getElementById('opfs-download-status');
    ...
    try {
      ...
      while (true) {
        ...
        if (totalBytes > 0) {
          ...
          statusText.innerText = `${(receivedBytes / (1024 * 1024)).toFixed(1)} MB / ${(totalBytes / (1024 * 1024)).toFixed(1)} MB`;
        } else {
          statusText.innerText = `${(receivedBytes / (1024 * 1024)).toFixed(1)} MB downloaded...`;
        }
      }

      statusText.innerText = "Saving to OPFS Storage...";
      ...
      // Update the Storage Bar after save
      updateStorageQuota();

      return URL.createObjectURL(blob);

    } catch (err) {
      console.error("OPFS Download Error:", err);
      modal.style.display = 'none';
      return pdfUrl; // Fallback to raw URL on failure
    }
  }

  // =========================================================
  // 3. HOOK OPFS INTO PDF.JS APP ENGINE
  // =========================================================
  document.addEventListener('DOMContentLoaded', async () => {
    updateStorageQuota();

    // Intercept URL parameter passed to viewer.html?file=...
    const urlParams = new URLSearchParams(window.location.search);
    const originalFileUrl = urlParams.get('file');

    if (originalFileUrl) {
      // Stream, download into OPFS, and get internal Blob URL
      const opfsBlobUrl = await fetchAndStoreInOPFS(originalFileUrl);
      
      // Pass OPFS Blob URL to PDF.js viewer
      if (window.PDFViewerApplication) {
        window.PDFViewerApplication.open(opfsBlobUrl);
      }
    }
  });
</script>
